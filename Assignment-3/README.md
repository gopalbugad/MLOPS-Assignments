...
## Docker Deployment (Simple Local Setup)
...
```bash
cd /path/to/LangGraph-Course-freeCodeCamp
...
```bash
docker run --env-file .env langgraph-app:latest
```

### Step 4: Run Specific Agent

To run a different agent, add the command override:

```bash
# Run RAG_Agent (default)
docker run --env-file .env langgraph-app:latest python Agents/RAG_Agent.py

# Run Memory_Agent
docker run --env-file .env langgraph-app:latest python Agents/Memory_Agent.py

# Run Agent_Bot
docker run --env-file .env langgraph-app:latest python Agents/Agent_Bot.py
```

### Step 5: Interactive Mode

To run the container interactively and execute commands manually:

```bash
docker run -it --env-file .env langgraph-app:latest /bin/bash
...
# Remove an image
docker rmi langgraph-app:latest
